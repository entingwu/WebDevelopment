		...
		return {
			...
			getBrowseCategory: function(categoryId) {
				...
				$http.get(baseUrl + '/browse/categories/' + categoryId, {
					...
					console.log('got browse browsecategory', r);
					ret.resolve(r);
				}).error(function(err) {
					console.log('failed to get browse browsecategory', err);
					ret.reject(err);
				});
				return ret.promise;
			},
			...
			getBrowseCategoryPlaylists: function(categoryId, country) {
				...
				$http.get(baseUrl + '/browse/categories/' + categoryId + '/playlists?country=' + encodeURIComponent(country), {
					...
					console.log('got browse browsecategory playlists', r);
					ret.resolve(r);
				}).error(function(err) {
					console.log('failed to get browsecategory playlists', err);
					ret.reject(err);
				});
				return ret.promise;
			},
